...
df=pd.read_csv('ASN_arin_db_2-20-2019_unparsed.txt',error_bad_lines=False, header = None)
df.columns = ['result']
df_out = df['result'].str.split(': ', expand=True)
df_out = df_out[[0,1]]
df_out = df_out.set_index([0,(df_out[0] == 'ASHandle').cumsum().rename('row')])
...
df_out = df_out[['ASHandle','OrgID','ASName','ASNumber','RegDate','Comment','Updated','Source']]
df_out = df_out.applymap(lambda x: x.strip() if isinstance(x, str) else x)
df_out.to_csv('ASN_arin_db_2-20-2019_parsed.csv', header=True, index=False)
