...
df=pd.read_csv('Networks_arin_db_2-20-2019_unparsed.txt',error_bad_lines=False, header = None)
df['result'] = df[0].map(lambda x: x.lstrip('V6'))
df = df[['result']]
df_out = df['result'].str.split(': ', expand=True)
df_out = df_out[[0,1]]
df_out = df_out.set_index([0,(df_out[0] == 'NetHandle').cumsum().rename('row')])
...
df_out = df_out[['NetHandle','OrgID','Parent','NetName','NetRange','NetType','Comment','RegDate','Updated','Source']]
df_out = df_out.applymap(lambda x: x.strip() if isinstance(x, str) else x)
df_out.to_csv('Networks_arin_db_2-20-2019_parsed.csv', header=True, index=False)
